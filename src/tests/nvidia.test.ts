...
test('NVIDIA forwarding paces requests instead of bursting into a long RPM wait', async () => {
  clearRuntimeConfig();
  ...
  assert.ok(rateSleeps.length > 1);
  assert.ok(rateSleeps.every((milliseconds) => milliseconds <= 1715));
});
